...
@section('content')
    <div>
        <h1>{{$logo->id?"Edit slide":"Create slide"}}</h1>
        ...
        <form class="form form-horizontal"
              action="{{$logo->id?route('admin.slide.update',['slide'=>$logo->id]):route('admin.slide.store')}}"
              method="post"
              autocomplete="off"
                    ...
                    <div id="block-upload-image">
                        ...
                        <div>
                            <img id="showImageIcon"
                                 src="{{url('/admin/slide-list/image/'.$logo->path_img)}}"
                                 class="OpenImgUploadIcon" title="อัพโหลดรูป"
                                 style="display: {{isset($logo->path_img)?'inline-block':'none'}};width: 100%;min-height: 269px;max-height: 270px;">
                        </div>
                    </div>
                </div>
            </div>
